...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        # 添加这一行，确保 splitting_label 被正确初始化
        self.splitting_label = self.config['LABEL_FIELD']
    ...
    def load_inter_graph(self, file_name):
        ...
        cols = []
        if self.uid_field:
            cols.append(self.uid_field)
        if self.iid_field:
            cols.append(self.iid_field)
        if self.splitting_label:
            cols.append(self.splitting_label)
        
        # 确保 sep 参数不为空
        sep = self.config['field_separator']
        if not sep:  # 如果是空字符串，使用默认的 \t
            sep = '\t'
        
        # 读取文件
        try:
            self.df = pd.read_csv(inter_file, usecols=cols, sep=sep, engine='python')
            
            # 检查列是否都存在
            if not all(col in self.df.columns for col in cols):
                # 输出更有用的错误信息
                missing_cols = [col for col in cols if col not in self.df.columns]
                raise ValueError(f'File {inter_file} missing columns: {missing_cols}. Available columns: {self.df.columns.tolist()}')
        
        except Exception as e:
            print(f"读取文件失败: {e}")
            print("尝试不指定 usecols 读取...")
            self.df = pd.read_csv(inter_file, sep=sep, engine='python')
            print(f"成功读取文件，可用列: {self.df.columns.tolist()}")
            
            # 检查必要的列是否存在
            missing_cols = [col for col in cols if col not in self.df.columns]
            if missing_cols:
                raise ValueError(f'必要的列不存在: {missing_cols}')
